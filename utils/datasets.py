...
def img2label_paths(img_paths):
    ...
    return [x.replace(sa, sb, 1).rsplit('.', 1)[0] + '.txt' for x in img_paths]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        if hr_input== False:
            self.img_path = '' #zjq the path for 512*512 images
        else:
            self.img_path = '' #zjq the path for 1024*1024 images
        ...
        with open(path, "r") as file:
            ...
            for j in range(len(self.img_files)):
                self.img_files[j] = self.img_path + self.img_files[j].rstrip()
...
class LoadImagesAndLabels_sr(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with open(path, "r") as file:
            ...
            #         self.img_files.remove(i+'\n')
            for j in range(len(self.img_files)):
                self.img_files[j] = self.img_files[j].rstrip()  #self.img_path + self.img_files[j].rstrip() + '_co.png'
